...
test('slice', t => {
  ...
  t.deepEqual(processIterable(slice(-1, 4, oneTwoThreeFour)), [4], 'negative start index')
  t.deepEqual(processIterable(slice(0, -1, oneTwoThreeFour)), [1, 2, 3], 'negative end index')
  t.deepEqual(processIterable(slice(-3, -2, oneTwoThreeFour)), [2], 'negative indices')
  t.deepEqual(processIterable(slice(-10, -2, oneTwoThreeFour)), [1, 2], 'negative indices')
  t.deepEqual(processIterable(slice(-3, -3, oneTwoThreeFour)), [], 'equal negative indices')
  t.deepEqual(processIterable(slice(-1, -2, positiveIntegers)), [], 'when indices are negative and start index is greater return empty iterable')
  t.end()
})
